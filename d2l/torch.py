...
class MaskLM(nn.Module):
    ...
    def forward(self, X, pred_positions):
        """
        :param X: 来自Bert编码器的输入，shape= (batch_size,文本序列长度，词向量大小)
        :param pred_positions: shape= (batch_size,预测数量),
        :return:
        """
        num_pred_positions = pred_positions.shape[1] # 获取每个batch中预测多少个词
        pred_positions = pred_positions.reshape(-1) # 展成一维
        batch_size = X.shape[0]
        batch_idx = torch.arange(0, batch_size) # 上一行代码将预测的词语位置矩阵展成了一维，这里需要标记预测时每个batch对应位置
        # 假设batch_size=2，num_pred_positions=3
        # 那么batch_idx是np.array（[0,0,0,1,1,1]）
        batch_idx = torch.repeat_interleave(batch_idx, num_pred_positions)
        masked_X = X[batch_idx, pred_positions] # shape = （总的预测数量，1，词向量长度）
        masked_X = masked_X.reshape((batch_size, num_pred_positions, -1)) # shape = （批量大小，每个批量预测数量，词向量长度）
